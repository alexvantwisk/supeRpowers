...
`air` is a fast, opinionated R formatter (Rust) from Posit — the current-
generation format-on-save tool. Use it for new projects; keep `styler` on
existing projects unless migrating intentionally.

```bash
# Install (pick one):
#   - Standalone installer:  curl -LsSf https://github.com/posit-dev/air/releases/latest/download/air-installer.sh | sh
#   - Homebrew:              brew install air
#   - From R:                usethis::use_air()   # downloads the binary + writes air.toml
air format R/        # format all files in R/
air format --check . # CI gate: nonzero exit if not formatted
```

Project config: `air.toml` at package root (write it with `usethis::use_air()`).
Positron uses Air as its default R formatter. Pre-commit hook: see air docs.
